...
| `cli` | citty CLI (`oddjob` binary) | Lazy proxy on `lib/api.ts` to defer transport construction |
...
- **No package may import `@oddjob/server` except `cli`.** That keeps the dashboard, api-client, and providers separable.
