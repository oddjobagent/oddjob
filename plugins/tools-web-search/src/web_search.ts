// web_search dispatcher. Resolves the configured WebSearchService backend via
// engine config (`[builtin_tools.web_search] plugin = "..."`), materializes
// credentials from `provider_credentials`, then delegates to the provider.

import type { Static } from "typebox";
import { Type } from "typebox";
import type { AgentTool, AgentToolResult } from "@mariozechner/pi-agent-core";

import { checkEnvAllowlist } from "@oddjob/agent";
import type {
  LogEntry,
  PluginRegistry,
  ProviderCredential,
...
export const webSearchTool: Omit<ToolService, "kind"> = {
  ...
  build: (ctx: ToolBuildContext) =>
    createWebSearchTool({
      config: ctx.engine?.builtinTools?.webSearch,
      onLog: ctx.onLog,
      plugins: ctx.plugins,
      secrets: ctx.secrets,
      state: ctx.state,
